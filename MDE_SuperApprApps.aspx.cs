...
namespace LRCA
{
    public partial class MDE_SuperApprApps : PageBase
    {
        ...
        protected void showTable(Panel pnlName, Supervisor supervisor)
        {
            ...
            var AppId = supervisor.Id.ToString();
            var AcctNum = "";
            var AcctExp = "";

            List<clsAccreditations> lstAcct = new List<clsAccreditations>();
            lstAcct = AccreditationsDAL.SelectDynamicAccreditations("RoleId = 5 and ApplicationId = " + AppId + "", "AccreditationId");
            if (lstAcct != null)
            {
                if (lstAcct.Count > 0)
                {
            ...
            if(pnlName == pnlDisapproved)
            {
                ...
                strContent.Append("</td>");
                strContent.Append("<td width='10%'nowrap>");
                strContent.Append(supervisor.SupervisorPhone);
                strContent.Append("</td>");
            }
            else
            {
            ...
            if (pnlName != pnlDisapproved)
            {
                ...
                strContent.Append("<a class='btn btn-xs btn-success download' title='Download as PDF' href='/" + objcryptoJS.AES_encrypt("Acct_Certificate_5" + "_" + AppId, AppConstants.secretKey, AppConstants.initVec) + ".cert' target='_blank' >Generate Certificate</a>");
                strContent.Append("</td>");
            }
            //***************************************
            strContent.Append("<td width='5%' nowrap>");
