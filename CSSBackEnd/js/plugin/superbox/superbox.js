	...
	$.fn.SuperBox = function(options) {
		...
		return this.each(function() {
			...
			$('.superbox-list').click(function() {
				...
				//
				//superboximg.find('.btn btn-danger btn-sm').text(currentimg.data('img'));
			...
			$('.superbox').on('click', '.superbox-delete', function() {
				
				
				filepicker.remove($this.find('.superbox-img').data('img'), function(){});
				
					$.ajax({
		            type: "POST",
		            url: "MediaLib.aspx/CallRemove",
		            data: "{\"cgi\":\"" + $this.find('.superbox-img').data('img') + "\"}",
