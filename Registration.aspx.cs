    ...
    public partial class Registration : System.Web.UI.Page
    {
        ...
        protected void btnReset(object sender, EventArgs e)
        {
            ...
            if (EmailIn.Length > 0)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "CallNotify('This email address has been used by some other user. Please use another valid email!', '', 'danger', 'Registration.aspx');", true);
                }
                else
                {
                    if (LoginBAL.Registration(FName, LName, EmailIn))
                    {
                        ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "script", "CallNotify('Your Account has been created!', '', 'success', 'Registration.aspx');", true);  
                    }
                    else
                    {
